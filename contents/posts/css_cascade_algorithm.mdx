...
즉, CSS 에서는 하나의 요소에 여러 개의 스타일이 적용된 경우, 특정한 우선순위를 따라 최종 스타일이 결정됩니다.

이 우선순위는 **중요도(`!important`)**, **스타일의 유래(origin)**, **선택자의 명시성(specificity)**, **선언 순서(order)**, **상속(inheritance)** 등 다양한 요소에 의해 결정됩니다.
...
CSS 에서는 `@layer` 를 사용하여 origin 을 변경할 수 있습니다. <br/>
`@layer` 를 사용하면 CSS 의 sub-origin 을 변경하여 스타일의 우선순위를 조정할 수 있습니다. <br/>

<br />

각 origin 레벨에 대해 여러개의 cascade layer 가 있을 수 있고, <br/>
각 레벨의 cascade layer 는 만들어진 순서에 따라 서로 다른 우선순위를 가집니다. `나중에 만든 레이어가 우선순위가 높습니다.` <br/>
...
### 3.2 Layer 는 origin 내부의 우선순위 서브 시스템이다

CSS의 origin은 다음과 같은 순서로 적용됩니다.
